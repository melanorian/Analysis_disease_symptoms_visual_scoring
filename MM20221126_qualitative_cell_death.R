...
setwd("C:/Users/Mende012/Documents/Bioinformatics/Cell_death")

# load file
df <- read_xlsx("./MM20221125_summary_visual_score_cell_death.xlsx")

# drop rows not to be included in data analyis - excel file should contain column
# remove rows with information that's no longer needed (simpler to work with)
df <- df[!(df$Include == "-"),]
df <- subset(df, select = -c(Notes, Include))
